...
/*   Updated: 2025/06/14 17:01:40 by agarcia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		**args;
	const char	*expected = "./script\\\"quote.sh";

	args = cmd_parse("'./script\\\"quote.sh'");
	if (!args)
	{
		printf("Error: NULL\n");
		return (1);
	...
	for (int i = 0; args[i]; i++)
		printf("arg[%d]: [%s]\n", i, args[i]);
	// Check result and print in green if OK, red if not
	if (args[0] && strcmp(args[0], expected) == 0)
		printf("\033[0;32mResult OK: [%s]\033[0m\n", args[0]);
	else
		printf("\033[0;31mResult FAIL: [%s] (expected: [%s])\033[0m\n",
			args[0] ? args[0] : "NULL", expected);
	// Free memory
	for (int i = 0; args[i]; i++)
		free(args[i]);
	free(args);
